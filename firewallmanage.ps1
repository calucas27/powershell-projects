<#
    .SYNOPSIS
    SCRIPT 01: FirewallManage: By Chase Lucas

    .EXAMPLE
    ./firewallmanage.ps1
    
    .DESCRIPTION
    Contrary to many scripts we've worked with this semester, I've chosen to create a menu-based script here.  Options for managing the
    properties of the firewall will be prompted for by the script as the user continues, and handled by the Net-Firewall module.
     Users will be able to add new firewall rules, manage existing firewall rules, and search for an existing rule given a display name.  
     Profiles can also be managed.  The option is also included to create a blanket disable rule for all profiles, similar to how rules would be 
     completely blocked for the first 10-15 minutes of a CCDC competition.
#>
